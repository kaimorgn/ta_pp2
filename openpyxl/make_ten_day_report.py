...
def make_ten_day_report(save_dir, txt_path):
    ...
    ten_day_report_path = Path(
        "input/internship_training_report_10_day_format.xlsx"
    )
    save_path = save_dir / "internship_training_report_10day.xlsx"

    try:
        wb_obj, ws = load_worksheet(ten_day_report_path, "10day")
        ...
        snapshot_path = Path(
            f"./{base_date.strftime('%Y%m%d')}_snapshot.zip"
        )
        ...
        logger.error(
            f"実習報告書(10日間)の編集中にエラーが発生しました: {e}"
        )
        raise e
...
if __name__ == "__main__":
    ...
    txt_path = Path("./input/report_data_10day.txt")
    make_ten_day_report(save_dir, txt_path)
